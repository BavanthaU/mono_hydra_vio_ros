 ...
 * @brief  ROS Wrapper for mono_hydra_vio
...
bool KimeraVioRos::runKimeraVio() {
  ...
  // Then, create mono_hydra_vio from scratch.
  VLOG(1) << "Creating mono_hydra_vio.";
  if (use_rviz_) {
    CHECK(ros_display_);
...
bool KimeraVioRos::spin() {
  ...
  if (vio_params_->parallel_run_) {
    ...
    if (!restart_vio_pipeline_) {
      LOG(INFO) << "Shutting down ROS and mono_hydra_vio.";
      ros::shutdown();
    } else {
      LOG(INFO) << "Restarting mono_hydra_vio.";
    }
    ...
    LOG(INFO) << "Joining mono_hydra_vio thread.";
    vio_pipeline_handle.get();
    LOG(INFO) << "mono_hydra_vio thread joined successfully.";
...
bool KimeraVioRos::restartKimeraVio(std_srvs::Trigger::Request& request,
  ...
  if (!restart_vio_pipeline_) {
    ...
    response.message = "mono_hydra_vio restart requested.";
    response.success = true;
  } else {
    response.message = "mono_hydra_vio should already be restarting...";
